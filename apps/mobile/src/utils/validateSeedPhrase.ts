 ...
 */
export const validateSeedPhrase = ({ phrase, isPrivateKey, setError, setSecret }: validateSeedPhraseParams) => {
  setError('');
  const correctedSecret = correctMnemonic(phrase);

  if (isPrivateKey) {
    try {
      const privateKey = correctedSecret.toLowerCase().startsWith('0x') ? correctedSecret.slice(2) : correctedSecret;
