...
export function useOnboarding() {
    ...
    if (password) {
      ...
      const accountsToSave = selectedCurrentAddresses.reduce(
        (
          ...
          const isEvmAddressPresent =
            !!addressInfo?.ethereum?.address && existingAddresses.includes(addressInfo.ethereum.address);
      ...
      const selectedAccountsPubKeys = Object.keys(accountsToSave).reduce((acc, path) => {
        ...
        const pubkey = account?.isCosmosAddressPresent
          ...
          : account?.chainAddresses?.[primaryChain]?.pubKey ?? account?.chainAddresses?.[alternateChain]?.pubKey;
  ...
  const getAccountDetails = async (mnemonic: string) => {
    const walletAccounts = await KeyChain.getWalletsFromMnemonic(mnemonic, 5, '118', ChainInfos.cosmos.addressPrefix);
    ...
    const walletAccountsWithAdditionalAddresses = await Promise.all(
      walletAccounts.map(async (account) => {
        ...
        const evmAddress =
          evmWallet instanceof EthWallet ? evmWallet.getAccountWithHexAddress()[0]?.address : undefined;
  ...
  const getLedgerAccountDetailsForIdxs = async (app: LEDGER_NETWORK, idxs: number[]) => {
    ...
    const { primaryChainAccount, pathWiseAddresses } = await importLedgerAccountV2(app, idxs, undefined, {
      primaryChain,
      chainsToImport,
      chainInfos,
    });
  ...
  const getCustomLedgerAccountDetails = async (
    ...
    const { primaryChainAccount, pathWiseAddresses } = await importLedgerAccountV2(app, [], [customDerivationPath], {
      primaryChain,
      chainsToImport: [],
      chainInfos,
    });
