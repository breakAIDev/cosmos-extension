...
export function useCreateViewingKey() {
  ...
  return useCallback(
    async (
      ...
      if (!importKey) {
        try {
          const { txStatus: _txStatus, viewingKey: _key } = await signingSecretClient.createViewingKey(
            signerAddress,
            contractAddress,
            gasPriceSteps,
            {
        ...
        const validKey = await verifyViewingKey(lcdUrl ?? '', viewingKey ?? '', contractAddress, signerAddress);
