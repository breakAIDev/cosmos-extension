...
export namespace Wallet {
  ...
  export function useImportMultipleWalletAccounts() {
    ...
    return useCallback(
      async ({ mnemonic, selectedAddressIndexes, password }: importMultipleWalletAccountsParams) => {
      ...
      async (chain?: SupportedChain, ethWallet?: boolean) => {
        ...
        if (
          ...
          if (chainInfos[_chain]?.bip44?.coinType === '60') {
            const derivationPath = activeWallet.path ? activeWallet.path : `0'/0/${activeWallet.addressIndex}`;
