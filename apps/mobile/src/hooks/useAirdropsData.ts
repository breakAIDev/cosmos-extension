import { Key, useActiveWallet, useInitAirdropsEligibilityData } from '@leapwallet/cosmos-wallet-hooks';
...
export const useAirdropsData = () => {
  ...
  const fetchAirdropsData = useCallback(async () => {
    ...
    const selectedWallet = Object.values(allWallets ?? {}).filter((wallet: Key) => wallet.id === activeWallet?.id)?.[0];
