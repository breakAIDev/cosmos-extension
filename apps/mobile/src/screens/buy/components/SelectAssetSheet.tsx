...
const SelectAssetSheet = observer(({ isVisible, onClose, onAssetSelect, selectedAsset }: SelectAssetSheetProps) => {
  ...
  const assetList = useMemo<AssetProps[] | []>(
    () =>
      supportedAssets.filter(
        (asset: AssetProps) =>
          asset.symbol.toLowerCase().includes(searchTerm) || asset.chainName.toLowerCase().includes(searchTerm),
      ),
  ...
  return (
    <BottomModal isOpen={isVisible} onClose={onClose} fullScreen title='Select token to buy' className='!p-6'>
            ...
            assetList.map((asset, index) => (
              <>
                <AssetCard
                  ...
                  isSelected={asset.symbol === selectedAsset?.symbol && asset.chainId === selectedAsset?.chainId}
                />
              </>
            ))}
        </div>
