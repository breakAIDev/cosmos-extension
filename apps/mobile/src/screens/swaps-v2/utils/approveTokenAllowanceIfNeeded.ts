...
export async function approveTokenAllowanceIfNeeded(
  ...
  try {
    ...
    if (currentAllowance.lt(weiAmountBN)) {
      ...
      const gasUsed = await SeiEvmTx.SimulateTransaction(tokenAddress, '0', evmJsonRpc ?? '', data, undefined, owner);
    ...
    throw new Error('Approval for token failed' + ((error as Error)?.message ? `: ${(error as Error).message}` : ''));
  } finally {
    // setShowLedgerPopup(false)
  }
}
