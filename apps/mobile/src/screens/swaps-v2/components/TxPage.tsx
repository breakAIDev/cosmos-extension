...
import { RoutingInfo, useExecuteTx, useHandleTxProgressPageBlurEvent, useOnline, useTransactions } from '../hooks';
...
export const TxPage = observer(
  ({
    ...
    const handleSwapAgainClick = useCallback(() => {
      ...
      if (initialSourceChain && initialDestinationChain && initialSourceToken && initialDestinationToken) {
        const srcChainId = String(initialSourceChain.chainId);
        const srcTokenId = getKeyToUseForDenoms(String(initialSourceToken.coinMinimalDenom), srcChainId);
        const destChainId = String(initialDestinationChain.chainId);
        const destTokenId = getKeyToUseForDenoms(String(initialDestinationToken.coinMinimalDenom), destChainId);
    ...
    return (
      ...
      >
            ...
            <div className='flex flex-col items-center gap-y-3'>
              ...
              <Text size='sm' color='text-secondary-800' className='font-normal text-center'>
                {isLoading
                  ...
                  : isSuccessFull
                  ? `You swapped ${formatTokenAmount(inAmount, sourceToken?.symbol)} for ${formatTokenAmount(
                      amountOut,
                      destinationToken?.symbol,
                    )}`
                  : failedActionWasSwap
        ...
        <div className=' flex flex-row items-center justify-between gap-4 absolute bottom-0 left-0 right-0 p-6 max-[350px]:!px-4 !z-[1000]'>
          <Button className={'flex-1'} variant={'mono'} onClick={handleHomeBtnClick} disabled={isHomeButtonDisabled}>
            Home
          </Button>
