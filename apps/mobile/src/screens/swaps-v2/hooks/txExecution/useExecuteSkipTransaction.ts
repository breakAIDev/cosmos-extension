...
export function useExecuteSkipTransaction({
  ...
  return useCallback(
    async (messages: SkipMsgWithCustomTxHash[] | undefined, route: SkipRouteResponse | undefined) => {
      ...
      if ('evm_tx' in messageObj) {
        ...
        try {
          if (!txHash && fee) {
            const wallet = (await getWallet(sourceChain?.key ?? ('' as SupportedChain), true)) as unknown as EthWallet;

            const seiEvmTx = SeiEvmTx.GetSeiEvmClient(wallet, evmJsonRpc ?? '', Number(message.chain_id));
            ...
            if (isLedgerTypeWallet) {
              ...
              if (!message?.required_erc20_approvals || message?.required_erc20_approvals.length === 0) {
      ...
      if (!txHash && fee) {
        ...
        const { senderAddress, encodedMessage } = getMessageMetadataForSigning(message.msg_type_url, messageJson);
        ...
        try {
          ...
          if (isLedgerTypeWallet) {
            if (sourceChain && ledgerEnabledEvmChainsIds.includes((sourceChain?.chainId as string) ?? '')) {
            ...
            const signedDoc = await (wallet as OfflineDirectSigner).signDirect(senderAddress, formattedSignDoc);
        ...
        try {
          ...
          try {
            const { transactionHash, code: txCode, codespace } = await txClient.broadcastTx(txBytesString as string);
