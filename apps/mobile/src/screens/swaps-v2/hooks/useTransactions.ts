...
export function useTransactions(routingInfo: RoutingInfo) {
  const { groupedTransactions: skipGroupedTransactions, actions: skipActions } = useSkipTransactions(
  ...
  const { groupedTransactions: mosaicGroupedTransactions, actions: mosaicActions } = useMosaicTransactions(
