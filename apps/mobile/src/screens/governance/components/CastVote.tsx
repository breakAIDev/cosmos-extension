...
// Import your custom RN components
...
export const CastVote = observer((props: CastVoteProps) => {
  const {
    style,
    proposalId,
    refetchVote,
  ...
  const defaultGasPrice = useDefaultGasPrice(denoms, { activeChain, selectedNetwork });
  ...
  // ...useEffect logic remains the same, just without web-only parts...
  ...
  useEffect(() => {
    (async function () {
      ...
      if (feeDenom.coinMinimalDenom === 'uosmo' && activeChain === 'osmosis') {
        ({ low, medium, high } = await getOsmosisGasPriceSteps(lcdUrl ?? '', allChainsGasPriceSteps));
  ...
  }, [activeChain, gasLimit, feeDenom.coinMinimalDenom, gasPriceOption.option, nativeFeeDenom?.coinMinimalDenom, hasToCalculateDynamicFee, lcdUrl, allChainsGasPriceSteps, getFeeMarketGasPricesSteps]);
  ...
  const handleGasPriceOptionChange = useCallback((value: GasPriceOptionValue, feeBaseDenom: FeeTokenData) => {
  ...
  const submitVote = useCallback(
    ...
    [activeChain, customFee, feeDenom, getWallet, modifiedCallback, vote],
  );
      ...
      try {
        ...
        if (result !== null && !cancelled) {
          ...
          setRecommendedGasLimit(
            String(Number(_estimate) * (activeChainCosmosSDK === CosmosSDK.Version_Point_47 ? 1.5 : 1)),
          );
          if (firstTime.current) {
            setGasLimit(String(Number(_estimate) * (activeChainCosmosSDK === CosmosSDK.Version_Point_47 ? 1.5 : 1)));
  ...
  }, [activeChainCosmosSDK, proposalId, simulateVote, isProposalInVotingPeriod, feeDenom.coinMinimalDenom]);
  ...
  }, [defaultGasPrice.gasPrice, defaultGasPrice.gasPrice.denom]);

  // Use moti for animating modal or sheets if desired
