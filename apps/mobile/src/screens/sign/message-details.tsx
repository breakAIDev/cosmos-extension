...
const tokenToString = async (token: Token, restUrl: string, chainId: string) => {
  try {
    ...
    return `${formatBigNumber(new BigNumber(token.quantity).dividedBy(10 ** trace.coinDecimals))} ${trace.coinDenom}`;
  } catch {
...
export const getMessageDetails = async (message: ParsedMessage, restUrl: string, chainId: string) => {
  switch (message.__type) {
    case ParsedMessageType.AuthzExec:
    ...
    case ParsedMessageType.BankMultiSend:
      return Promise.resolve(`Send ${message.inputs.length} coins to ${message.outputs.length} recipients`);
    case ParsedMessageType.BankSend: {
      return `Send ${await tokensToString(message.tokens, restUrl, chainId)} to ${sliceAddress(message.toAddress)}`;
    }
    case ParsedMessageType.FeeGrantGrantAllowance:
      return Promise.resolve(
        `Grant ${formatAllowanceOption(message.allowance)} allowance to ${sliceAddress(message.grantee)}`,
      );
    case ParsedMessageType.IbcPacketReceive:
      return Promise.resolve(
        `Receive IBC packet from ${sliceAddress(message.sourcePort)}/${sliceAddress(message.sourceChannel)}`,
      );
    case ParsedMessageType.FeeGrantRevokeAllowance:
      return Promise.resolve(`Revoke allowance from ${sliceAddress(message.grantee)}`);
    case ParsedMessageType.GammCreatePool:
      return `Create a Balancer pool with ${await tokensToString(message.tokens, restUrl, chainId)} assets`;
    case ParsedMessageType.GammJoinPool:
    ...
    case ParsedMessageType.GammExitPool:
      return `Exit pool ${message.poolId} with ${message.shares} shares in return for ${await tokensToString(
        message.tokens,
        restUrl,
        chainId,
      )} assets`;
    ...
    case ParsedMessageType.PMSwapExactIn: {
      ...
      return `Swap ${await tokenToString(message.tokenIn, restUrl, chainId)} for ${await tokenToString(
        tokenOut,
        restUrl,
        chainId,
      )}`;
    ...
    case ParsedMessageType.GovSubmitProposal:
      return Promise.resolve(
        `Submit proposal with deposit of ${await tokensToString(message.initialDeposit, restUrl, chainId)}`,
      );
    case ParsedMessageType.GovVote: {
      let proposalId;
    ...
    case ParsedMessageType.GovDeposit:
      return Promise.resolve(
        `Deposit ${await tokensToString(message?.amount ?? [], restUrl, chainId)} on proposal ${message.proposalId}`,
      );
    case ParsedMessageType.IbcSend:
    ...
    case ParsedMessageType.LockupLock:
      return `Lock ${await tokensToString(message.tokens, restUrl, chainId)} for ${message.duration} seconds`;
    case ParsedMessageType.LockupUnlock:
    ...
    case ParsedMessageType.StakingCreateValidator:
      return Promise.resolve(`Create ${message.moniker} validator with commission rate ${message.rate}`);
    case ParsedMessageType.StakingEditValidator:
    ...
    case ParsedMessageType.StakingBeginRedelegate:
      return `Redelegate ${await tokenToString(
        ...
        restUrl,
        chainId,
      )} from ${sliceAddress(message.sourceValidatorAddress)} to ${sliceAddress(message.destinationValidatorAddress)}`;
    case ParsedMessageType.StakingCancelUnbondingDelegation:
      return Promise.resolve(
    ...
    case ParsedMessageType.SuperfluidLockAndDelegate:
      return `Lock ${await tokensToString(message.tokens, restUrl, chainId)} and delegate to ${sliceAddress(
    ...
    case ParsedMessageType.StakeIBCRedeemStake:
      return Promise.resolve(`Redeem liquid stake of ${message.quantity} units to ${sliceAddress(message.receiver)}`);
    case ParsedMessageType.StakeIBCClaimUndelegatedTokens:
      return Promise.resolve('Claim undelegated tokens');
    case ParsedMessageType.StakeIBCRegisterHostZone:
      return Promise.resolve(
        `Register host zone via ${sliceAddress(message.account)} with host denomination ${message.hostDenom}`,
      );
    case ParsedMessageType.StakeIBCRebalanceValidators:
      return Promise.resolve(
    ...
    case ParsedMessageType.StakeIBCRestoreInterchainAccount:
      return Promise.resolve(
        `Restore interchain account ${sliceAddress(message.creator)} of type ${message.accountType}`,
      );
    case ParsedMessageType.StakeIBCUpdateValidatorSharesExchRate:
      return Promise.resolve(
        `Update liquid staking validator shares exchange rate for ${sliceAddress(message.validatorOperatorAddress)}`,
      );
    case ParsedMessageType.ClaimReward:
...
export const useMessageDetails = (message: ParsedMessage | undefined, restUrl: string, activeChain: SupportedChain) => {
