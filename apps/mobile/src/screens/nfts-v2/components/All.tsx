...
export const All = observer(({ searchedText, selectedSortsBy, nftStore }: AllProps) => {
  ...
  const sortedCollectionChains = nftStore.getSortedCollectionChains(disabledNFTsCollections, hiddenNftStore.hiddenNfts);
      ...
      (selectedSortsBy.length ? selectedSortsBy : sortedCollectionChains).forEach((chain) => {
        ...
        const searchedNfts = tempNfts.filter((nft) => {
          return (
            ...
            `#${nft.tokenId ?? nft.domain}`.trim().toLowerCase().includes(searchedText.toLowerCase())
          );
        });
  ...
  return (
    <>
      ...
      {sortedCollectionChains.map((chain) => {
        ...
        return <ChainHeaderCollectionCard key={chain} chain={chain} nfts={nfts} nftsCount={nfts.length} />;
      })}
    </>
  );
});
