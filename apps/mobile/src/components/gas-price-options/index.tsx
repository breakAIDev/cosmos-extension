...
import {
  GasPriceOptionsContext,
  GasPriceOptionsContextType,
  useGasPriceContext,
} from './context';
...
export const useDefaultGasPrice = (
  ...
  const { gasPrice: solanaGasPrice } = useGetSolanaGasPrices(
    activeChain,
    options?.selectedNetwork,
  );
  const { gasPrice: suiGasPrice } = useGetSuiGasPrices(
    activeChain,
    options?.selectedNetwork,
  );
  ...
  const defaultPrice = useMemo(() => {
    const feeDenom =
      options?.feeDenom ?? (nativeFeeDenom as ExtendedNativeDenom);
...
const GasPriceOptions = observer(
  ({
    ...
    const activeChain =
      chain ?? (activeChainStore.activeChain as SupportedChain);
    const selectedNetwork = network ?? selectedNetworkStore.selectedNetwork;

    const chainGasPriceOptionsStore = gasPriceOptionsStore.getStore(
      activeChain,
      selectedNetwork,
    );

    const allTokensLoading = rootBalanceStore.getLoadingStatusForChain(
      activeChain,
      selectedNetwork,
    );
    const spendableBalancesForChain =
      rootBalanceStore.getSpendableBalancesForChain(
        activeChain,
        selectedNetwork,
      );

    const chainInfo = chainInfoStore.chainInfos[activeChain];
    const evmBalance = evmBalanceStore.evmBalanceForChain(
      activeChain,
      selectedNetwork,
    );
    ...
    const suiBalance = suiCoinDataStore.getSuiBalances(
      activeChain,
      selectedNetwork,
    );
    ...
    const finalRecommendedGasLimit =
      chainGasPriceOptionsStore.finalRecommendedGasLimit;
    const hasToCalculateDynamicFee =
      chainGasPriceOptionsStore.hasToCalculateDynamicFee;

    const feeTokens = feeTokensStore.getStore(
      activeChain,
      selectedNetwork,
      isSeiEvmTransaction,
    );
    const chainNativeFeeTokenData = feeTokens?.data?.[0];
    const isPayingFeeInNonNativeToken =
      feeTokenData?.ibcDenom !== chainNativeFeeTokenData?.ibcDenom;
    ...
    useEffect(() => {
      const fn = async () => {
        const dappFeeDenomData = feeTokens?.data?.find(
          a => a.ibcDenom === initialFeeDenom,
        );
        ...
        if (
          ...
          const baseGasPriceStep =
            await gasPriceStepForChainStore.getGasPriceSteps(
              activeChain,
              selectedNetwork,
            );
          const { lcdUrl } = await chainApisStore.getChainApis(
            activeChain,
            selectedNetwork,
          );
        ...
        if (hasToCalculateDynamicFee && feeTokenData) {
          ...
          const gasPriceStep =
            await feeMarketGasPriceStepStore.getFeeMarketGasPricesSteps({
              chain: activeChain,
    ...
    useEffect(() => {
      ...
      if (
        ...
        chainGasPriceOptionsStore.setFinalRecommendedGasLimit(
          newRecommendedGasLimit,
        );
    ...
    useEffect(() => {
      const gasPriceBN = new BigNumber(
        gasPriceOption.gasPrice.amount.toFloatApproximation(),
      );
      ...
      if (!feeTokenAsset && feeTokenData?.denom.coinDenom) {
        return setError(
          `You do not have any ${feeTokenData?.denom.coinDenom} tokens`,
        );
      }
      ...
      const hasToChangeDecimals =
        (isSeiEvmTransaction &&
          feeTokenData?.denom?.coinMinimalDenom === 'usei') ||
        chainInfo?.evmOnlyChain;
      ...
      const amount = gasPriceBN
        ...
        .dividedBy(
          10 **
            (hasToChangeDecimals ? 18 : feeTokenData?.denom?.coinDecimals ?? 8),
        );
      ...
      if (
        ...
        ((isIbcDenom && feeTokenAsset?.ibcDenom === feeTokenData?.ibcDenom) ||
          feeTokenAsset?.coinMinimalDenom ===
            feeTokenData.denom?.coinMinimalDenom)
      ) {
      ...
      const foundFeeTokenData = feeTokens?.data?.find(
        feeToken =>
          !!allTokens?.find((token: Token) => {
            if (token.ibcDenom) {
              return (
                token.ibcDenom === feeToken?.ibcDenom && tokenHasBalance(token)
              );
            } else {
              return (
            ...
            feeTokens?.data?.find(
              feeToken =>
                !!allTokens?.find((token: Token) => {
                  if (token.ibcDenom) {
                    return (
                      token.ibcDenom === feeToken?.ibcDenom &&
                      tokenHasBalance(token)
                    );
                  } else {
                    return (
                      token.coinMinimalDenom ===
                        feeToken?.denom?.coinMinimalDenom &&
                      tokenHasBalance(token)
                    );
                  }
                }),
        ...
        <GasPriceOptionsContext.Provider
          value={{
            ...
            viewAdditionalOptions:
              chainGasPriceOptionsStore.viewAdditionalOptions,
            ...
            userHasSelectedToken:
              chainGasPriceOptionsStore.userHasSelectedToken,
            setUserHasSelectedToken: v =>
              chainGasPriceOptionsStore.setUserHasSelectedToken(v),
...
export const calculateFeeAmount = ({
  ...
  if (isSolana) {
    if (isNaN(amount.toNumber())) {
      ...
      amount = amount.plus(
        new BigNumber(5000).dividedBy(10 ** feeDenom.coinDecimals),
      );
    }
  }
  if (isSui) {
    amount = amount.plus(
      new BigNumber(computedGas ?? 0).dividedBy(10 ** feeDenom.coinDecimals),
    );
  }
  return {
    amount,
  ...
  return (
    <>
      <div className={classNames('grid grid-cols-3 gap-[5px]', className)}>
        {Object.entries(feeTokenData?.gasPriceStep ?? {}).map(
          ([level, gasPrice]) => {
            const isSelected = value.option === level;
            const gasPriceBN = new BigNumber(gasPrice);
            const estimatedGasLimit =
              gasLimit ??
              defaultGasEstimatesStore.estimate?.[activeChain]
                .DEFAULT_GAS_TRANSFER ??
              DefaultGasEstimates.DEFAULT_GAS_TRANSFER;
            const { amount, formattedAmount, isVerySmallAmount } =
              calculateFeeAmount({
                gasPrice: gasPriceBN,
            ...
            return (
              <label
                ...
                title={
                  amountInFiat
                    ? `${
                        currencyDetail[preferredCurrency].symbol
                      } ${amountInFiat.toFixed(8)}`
                    : level
                }
              >
                <input
                ...
                {amountInFiat ? (
                  <span
                    className={`text-muted-foreground text-xs transition-all text-center mt-0.5`}
                  >
...
GasPriceOptions.AdditionalSettingsToggle = function AdditionalSettingsToggle({
  ...
  const { viewAdditionalOptions, setViewAdditionalOptions } =
    useGasPriceContext();
...
GasPriceOptions.AdditionalSettings = observer(
  ({
    ...
    // hardcoded
    const hasToCalculateDynamicFee = useHasToCalculateDynamicFee(
      activeChain,
      selectedNetwork,
    );
    ...
    useEffect(() => {
      setGasLimitInputValue(() => {
        const limit =
          gasLimit.toString() || recommendedGasLimit?.toString() || '100000';
        return Math.round(
          Number(limit) *
            (considerGasAdjustment
              ? gasAdjustmentStore.getGasAdjustments(activeChain)
    ...
    if (
      !feeTokenAsset &&
      (!eligibleFeeTokens || eligibleFeeTokens?.length === 0)
    ) {
      if (allTokensStatus === 'error') {
        return (
          <div className="w-full z-0 p-3">
            <p className="text-sm dark:text-gray-400 text-gray-700">
              Failed to load your tokens, please reload the extension and try
              again
            </p>
          </div>
        );
      ...
      if (allTokensStatus === 'success' && viewAdditionalOptions) {
        return (
          <div className="w-full z-0 p-3">
            <p className="text-sm dark:text-gray-400 text-gray-700">
              You do not have any tokens that can be used to pay transaction
              fees.
            </p>
          </div>
        );
    ...
    const handleTokenSelect = async (
      selectedMinimalDenom: string,
      selectedIbcDenom?: string,
    ) => {
    ...
    const handleGasLimitOnChange = (e: ChangeEvent<HTMLInputElement>) => {
      ...
      if (
        !isGasLimitInvalid(gasLimitInputValue) &&
        _gasLimit !== recommendedGasLimit
      ) {
        ...
        <div ref={ref} className={classNames('w-full p-4', className)}>
          ...
          <div className="mt-4">
            <div className="flex flex-col gap-3">
              ...
              {gasError ? (
                <p className="text-destructive-100 text-xs font-medium">
                  {gasError}
                </p>
              ) : showGasLimitWarning &&
                inputTouched &&
