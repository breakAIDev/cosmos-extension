...
export const DisplayFee: React.FC<DisplayFeeProps> = ({
  ...
  const displayFee = useMemo(() => {
    ...
    return {
      ...
      fiatValue: feeTokenFiatValue
        ? formatCurrency(amount.multipliedBy(feeTokenFiatValue))
        : '',
    };
  }, [
    gasLimit,
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onClick={() =>
          setShowFeesSettingSheet ? setShowFeesSettingSheet(true) : undefined
        }
      ...
      >
        ...
        {setShowFeesSettingSheet && (
          <CaretDown size={16} className="ml-1 text-muted-foreground" />
        )}
      </button>
    </div>
  );
