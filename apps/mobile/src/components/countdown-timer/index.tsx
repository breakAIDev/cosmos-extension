...
export default function CountDownTimer({ minutes = 0, seconds = 0, setRevealed, setPassword }: ICountdown) {
  ...
  return (
    <span className='text-gray-200 font-bold text-xs'>{`${time.minutes.toString().padStart(2, '0')}:${time.seconds
      .toString()
      .padStart(2, '0')}`}</span>
  );
}
