...
import {
  ManageChainSettings,
  manageChainsStore,
} from 'stores/manage-chains-store';
...
export const AggregatedNullComponents = observer(function ({
  ...
  const aggregatedChains =
    aggregatedChainsStore.aggregatedChainsData as SupportedChain[];
  ...
  const chainsToFetch = useMemo(() => {
    return aggregatedChains.reduce((acc: ManageChainSettings[], chain) => {
      ...
      const managedChain = managedChains.find(
