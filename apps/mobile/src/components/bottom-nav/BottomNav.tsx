...
const BottomNav = observer(
  ({ label, disabled: disabledAll }: BottomNavProps) => {
    ...
    const bottomNavItems = useMemo(() => {
      ...
      return [
        ...
        },
        {
          ...
          show: true,
          disabled:
            activeChainInfo?.disableStaking || activeChainInfo?.evmOnlyChain,
    ...
    return (
      <div className="flex absolute justify-around bottom-0 h-[65px] w-full rounded-b-lg z-[0] bg-white-100 dark:bg-gray-950 shadow-[0_-8px_20px_0px_rgba(0,0,0,0.04)] dark:shadow-[0_-8px_20px_0px_rgba(0,0,0,0.26)]">
        ...
        {bottomNavItems
          ...
          .map(
            (
              { label, icon, path, shouldRedirect, redirectHandler, disabled },
              idx,
            ) => {
