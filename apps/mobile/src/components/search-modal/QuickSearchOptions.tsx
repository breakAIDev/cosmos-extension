import { OptionPlatformConfig, QuickSearchOption, useFeatureFlags } from '@leapwallet/cosmos-wallet-hooks';
...
const QuickSearchOptionsView = ({
  ...
  return suggestionsList.length ? (
    <ul className='px-3 flex flex-col gap-2 overflow-y-auto h-[420px] outline-none' tabIndex={0}>
      {suggestionsList.map(({ action_name, action_icon_url, action_light_icon_url, extension_config, tags }, index) => {
        ...
        return (
          <li
            ...
            onClick={() => handleOptionClick(extension_config as OptionPlatformConfig, index, action_name)}
          >
            ...
            {extension_config?.action_type === 'redirect-external' ? (
              <img src={Images.Misc.OpenLink} className='w-[12px] h-[12px] -ml-[4px] mt-[8px] invert dark:invert-0' />
            ) : null}
          </li>
        );
      })}
    ...
    <div className='w-full h-[420px] flex flex-col items-center justify-center'>
      ...
      <div className='font-bold text-gray-800 dark:text-white-100 text-base mt-3'>No result found</div>
