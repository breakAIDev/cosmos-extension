  ...
  const activeSuggestions = useMemo(() => {
    const extensionSuggestions =
      suggestions?.filter(({ visible_on }) => {
        if (
          visible_on.platforms.includes('All') ||
          visible_on.platforms.includes('Extension')
        ) {
          return true;
        }
    ...
    return extensionSuggestions.filter(({ visible_on }) => {
      if (
        visible_on.chains.includes('All') ||
        visible_on.chains.includes(activeChain)
      ) {
        return true;
      }
  ...
  const filteredSuggestions = useMemo(() => {
    if (searchedText.trim()) {
      ...
      if (dappsList && dappsList?.dapps?.length) {
        _filteredDapps = getFilteredDapps(
          dappsList.dapps,
          dappsList.types,
          searchedText,
        );
  ...
  const handleNoRedirectActions = (actionName: string) => {
    switch (actionName) {
      ...
      case 'Hide Balances': {
        ...
        setAlertMessage(
          `Balances ${!hideAssetsStore.isHidden ? 'Hidden' : 'Visible'}`,
        );
        setShowAlert(true);
        break;
      }
        ...
        >
          <div className="w-full py-3 px-6 border-b dark:border-b-gray-900 flex items-center gap-3">
            <img
              ...
              src={Images.Misc.SearchModalGlass}
              alt=""
            />
            <input
              type="text"
