...
const NewChainSupportTooltip = () => {
  ...
  const handleAddChainClick = useCallback(
    (chain: string) => {
      const item = customChains.find(
        customChain => customChain.chainRegistryPath === chain,
      );
      let chainKey;
  ...
  const handleSwitchChainClick = useCallback(
    (chainRegistryPath: string) => {
      ...
      if (chainKey) {
        ...
        captureException(
          `${chainRegistryPath} chain not found when clicked on banners`,
          {
            tags: uiErrorTags,
          },
        );
  ...
  const handleCTAClick = useCallback(() => {
    ...
    switch (toolTipData?.ctaAction?.type) {
      case 'redirect-internally': {
        navigate(
          `${toolTipData?.ctaAction.redirectUrl}&toolTipId=${toolTipData?.id}`,
        );
        break;
      }
  ...
  return (
    <>
      ...
      >
        ...
        {toolTipData?.imgUrl && (
          <img
            src={toolTipData?.imgUrl}
            className="rounded-lg w-full h-[72px]"
            alt="tooltip-img"
          />
        )}
        ...
        <div className="flex row w-full justify-between items-center">
          <button
            ...
            className="bg-green-600 rounded-full flex py-2 px-3"
          >
      ...
      <AddFromChainStore
        ...
        newAddChain={
          customChains.find(d => d.chainName === newChain) as ChainInfo
        }
      />
    </>
  );
