...
const ActionButton = React.memo(
  ({ type, onClick, className }: HeaderAction) => {
    const isInExpandView =
      browser.extension.getViews({ type: 'tab' }).length > 0;
        ...
        return (
          <div
            className={classNames(
              'flex flex-row justify-center items-center !p-0',
              className,
            )}
          >
            <div
              className={classNames(
                ...
                isSidePanelSupported()
                  ? '[&_button]:pl-[0.6rem] [&_button]:pr-[0.5rem]'
                  : null,
              )}
            >
              <Buttons.Nav
