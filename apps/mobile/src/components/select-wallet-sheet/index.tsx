...
export default function SelectWalletSheet({
  ...
  const walletAddress =
    currentWalletInfo?.wallets?.[0]?.addresses?.[activeChain];
        ...
        <div>
          ...
          <div className="flex flex-col rounded-2xl bg-white-100 dark:bg-gray-900 h-fit max-h-[250px] overflow-y-auto">
            {walletsList.map((wallet, index, array) => {
              ...
              if (wallet.id === currentWalletInfo?.wallets?.[0]?.id)
                return null;
              let walletLabel = '';
              ...
              const walletName =
                ...
                !LEDGER_NAME_EDITED_SUFFIX_REGEX.test(wallet.name)
                  ? `${walletLabels[wallet.walletType]} Wallet ${
              ...
              return (
                <div className="relative min-h-[56px] w-full" key={wallet.id}>
                  <Card
                    onClick={async () => {
                      const walletIds = currentWalletInfo?.wallets.map(
                        wallet => wallet.id,
                      );
                    ...
                    iconSrc={
                      activeWallet?.id === wallet.id
                        ? Images.Misc.CheckCosmos
                        : undefined
                    }
                    imgSrc={Images.Misc.getWalletIconAtIndex(
                      wallet.colorIndex,
                      wallet.watchWallet,
                    )}
                    isRounded={true}
