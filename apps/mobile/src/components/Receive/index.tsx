...
function BtcButton() {
  ...
  return nomicBtcDeposit &&
    nomicBtcDeposit.ibcChains.includes(activeChainInfo?.key) ? (
    <div
      className="mt-2"
      onClick={() => {
        ...
        <div className="flex flex-col items-center">
          ...
          {ON_RAMP_SUPPORT_CHAINS.includes(activeChain) && (
            ...
            >
              <OnboardCard
                imgSrc={
                  isDark ? Images.Logos.SwappedDark : Images.Logos.SwappedLight
                }
                iconSrc={rightArrow}
                isFilled
                isRounded
                size="lg"
                title={`Buy ${
                  activeChain === 'osmosis' ? 'AxlUSDC' : 'Crypto'
                } with Swapped Ramp`}
              />
            </div>
          )}
