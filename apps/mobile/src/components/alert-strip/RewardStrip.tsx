...
const RewardStrip = observer(() => {
  ...
  const faucetInactive = useMemo(
    () => faucet?.status === 'inactive',
    [faucet?.status],
  );
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setFaucet(
        data.data.find(
          (item: any) =>
            item.chain_id === chain?.chainId && item.status !== 'upcoming',
        ),
      );
    } catch (error: any) {
      captureException(error);
  ...
  const handleSubmit = useCallback(async () => {
    if (eligibility.code === 'CAPACITY_FULL') {
      query.set(
        'faucet-error',
        'Experiencing high traffic. Please try after few minutes.',
      );
      return;
    }
    if (eligibility.code === 'IP_LIMIT_REACHED') {
      query.set(
        'faucet-error',
        'You have reached the maximum limit to claim tokens.',
      );
      return;
    }
      ...
      <div onClick={handleSubmit} className="mb-4 cursor-pointer relative px-7">
        <img
          ...
          className={classNames({
            '!h-[64px] rounded-lg object-cover': isSidePanel(),
          })}
        />
        <div className="flex flex-col justify-center absolute py-2.5 px-4 gap-y-[5px] top-0 h-[64px] w-[288px]">
          ...
          {!faucetInactive &&
            (eligibility.code !== 'ALREADY_CLAIMED' || successClaim) && (
              <div
                className={`flex ${
