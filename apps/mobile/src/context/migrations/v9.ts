// src/utils/storageMigrationV9.ts
import AsyncStorage from '@react-native-async-storage/async-storage';

import { ACTIVE_WALLET, KEYSTORE } from '../../services/config/storage-keys';

// Usage: await storageMigrationV9(storageObj);
export async function storageMigrationV9(storage: Record<string, any>) {
  const keyStore = storage[KEYSTORE];
  const activeWallet = storage[ACTIVE_WALLET];
  const walletEntries = Object.entries(keyStore);

  const hasZeroIndex = walletEntries.find(
    (wallet: [string, any]) => wallet[1].addressIndex === 0
  );
  ...
  // Save new keyStore and activeWallet to AsyncStorage
  await AsyncStorage.setItem(KEYSTORE, JSON.stringify(newWalletEntries));
