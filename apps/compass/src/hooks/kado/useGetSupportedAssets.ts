...
export function useGetSupportedAssets() {
  ...
  function filterData() {
    ...
    kadoAssets.map((asset: any) => {
      ...
      const denomData = denomsArray.find((denom) => denom.coinDenom.toLowerCase() === asset.symbol?.toLowerCase());
      if (chain) {
        assets.push({
          symbol: asset.symbol,
    ...
    transakAssets.map((asset: any) => {
      const existingAsset = assets.find((item) => item.symbol.toLowerCase() === asset.symbol?.toLowerCase());
      if (existingAsset) return;
      const chain = chainsArray.find((chain) => chain.key !== 'cosmos' && chain.chainName === asset.name);
      const denomData = denomsArray.find((denom) => denom.coinDenom.toLowerCase() === asset.symbol?.toLowerCase());
      if (chain) {
        assets.push({
          symbol: asset.symbol,
