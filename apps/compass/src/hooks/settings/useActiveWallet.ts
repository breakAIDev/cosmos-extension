import { Key, useActiveWalletStore, useFeatureFlags, useIsCompassWallet } from '@leapwallet/cosmos-wallet-hooks';
...
export default function useActiveWallet() {
  ...
  const setActiveWallet = useCallback(
    async (wallet: Key | null) => {
      ...
      const activeChain: SupportedChain = isCompassWallet ? store[ACTIVE_CHAIN] ?? 'seiTestnet2' : lastEvmActiveChain;
