    ...
    function getBetaChains(updateStore?: boolean) {
      browser.storage.local
        ...
        .then(async (resp) => {
          ...
          let betaChains = Object.keys(_allChains).length > 0 ? { ..._allChains, ..._betaChains } : _betaChains;
          ...
          const enabledChains = Object.entries(ChainInfos).reduce((chainInfos, [chainKey, chainData]) => {
            ...
            if (isCompassWallet() && !['arctic-1', 'pacific-1', 'cosmoshub-4'].includes(chainData.chainId)) {
              return chainInfos;
