...
export const SignTransaction = observer(
  ({
    ...
    const assets = useMemo(() => {
      ...
      const addEvmDetails = hasToAddEvmDetails(isCompassWallet(), addressLinkState, chainInfo?.evmOnlyChain ?? false);
    ...
    useEffect(() => {
      ...
      async function compareAgainstBlockBaseFee() {
        ...
        if (latestBaseFeePerGas && new BigNumber(gasPriceOption.gasPrice.amount.toString()).lt(latestBaseFeePerGas)) {
          setGasPriceError('Max fee per gas is less than the block base fee. Please increase the gas fee.');
          return;
        }
    ...
    const handleApproveClick = async () => {
      try {
        ...
        if (error instanceof LedgerError) {
          ...
          error.message === txDeclinedErrorUser.message && handleRejectClick(navigate, txnData?.payloadId, donotClose);
          return;
        }
