...
const SelectProvider = observer(({ isVisible, onClose, onProviderSelect, providersQuote, asset }: Props) => {
    ...
    >
      <div className='flex flex-col gap-5 w-full'>
        {Object.entries(providersQuote)
          .sort(([k1, v1], [k2, v2]) => (parseFloat(v1.crypto ?? '0') > parseFloat(v2.crypto ?? '0') ? -1 : 1))
          .map(([key, value], index) => {
            return (
              <div
                key={key}
                className={cn('flex justify-between items-center w-full bg-secondary rounded-xl p-5 cursor-pointer', {
                  'border border-monochrome': index === 0,
                })}
                onClick={() => onProviderSelect(key as ServiceProviderEnum)}
              >
                <div className='flex gap-4 items-center'>
                  <img src={ProviderDetails[key as ServiceProviderEnum].image} className='w-10 h-10' />
