...
export const ReviewTransferSheet = observer(({ isOpen, onClose, setShowTxPage }: ReviewTransactionSheetProps) => {
      ...
      if (isSeiEvmTransaction || chains[sendActiveChain]?.evmOnlyChain) {
        ...
        confirmSendEth(
          ...
          parseInt(userPreferredGasPrice?.amount?.toString() ?? ''),
          {
            ...
            contractAddress: hasToUsePointerLogic ? pointerAddress : selectedToken.coinMinimalDenom,
  ...
  return (
    <>
      ...
      <LedgerConfirmationPopup showLedgerPopup={showLedgerPopup && !txError} onCloseLedgerPopup={onCloseLedgerPopup} />
    </>
  );
});
