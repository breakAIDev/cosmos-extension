...
export const ReviewTransfer = observer(({ setShowTxPage }: { setShowTxPage: (val: boolean) => void }) => {
  ...
  const isReviewDisabled = useMemo(() => {
    ...
    if (addressWarning.type === 'link') {
      return (
        ['loading', 'success'].includes(addressLinkState) || featureFlags?.link_evm_address?.extension === 'disabled'
      );
    }
  ...
  return (
    <>
      <div className='mt-4 flex flex-col gap-5'>
        ...
        {showLoadingMessage ? <p className='text-yellow-600 text-center mt-4'>{showLoadingMessage}</p> : null}
      </div>
