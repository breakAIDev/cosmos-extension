...
async function sendMessage(message: { type: string; payload: unknown; status: 'success' | 'failed' }) {
  try {
...
const ApproveConnection = () => {
  ...
  return (
    <ApproveConnectionWrapper origin={originValue}>
      <div className={'flex items-center justify-between gap-4 bg-secondary-100 rounded-2xl py-5 px-4'}>
        <div className='flex flex-col gap-3'>
          <span className={'text-sm text-muted-foreground font-medium'}>Connecting {walletName}</span>
      ...
      <div className='flex flex-col gap-4 rounded-xl p-5 bg-secondary-100 text-xs text-muted-foreground font-medium'>
        ...
        <span className='text-xs text-muted-foreground font-medium'>This app won&apos;t be able to</span>
        <span className='flex items-center gap-2'>
