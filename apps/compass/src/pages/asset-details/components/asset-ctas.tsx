...
export const AssetCtas = ({ denomInfo, assetsId }: { denomInfo: NativeDenom; assetsId?: string }) => {
  ...
  return (
    <div className='flex gap-6 p-4 justify-center w-full'>
      ...
      {isStakeDisabled ? null : (
        <ClickableIcon label='Stake' icon={DollarIcon} onClick={handleStakeClick} disabled={isStakeDisabled} />
      )}
    </div>
  );
};
