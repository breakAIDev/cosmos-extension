...
const getStepName = (config: { currentStep: number; walletName: ImportWalletType; loading: boolean }): StepName => {
...
export const ImportWalletProvider = observer(({ children }: { children: React.ReactNode }) => {
  ...
  const onOnboardingCompleted = async (password: Uint8Array) => {
    try {
      setLoading(true);
      if (isLedger) {
        const walletAccounts = isLedger ? ledgerWalletAccounts.concat(ledgerCustomWalletAccounts) : hotWalletAccounts;
        ...
        const [privateKey, userInfo] = await Promise.all([socialLogin.getPrivateKey(), socialLogin.getUserInfo()]);

        if (!privateKey) {
          throw new Error('Private key not found');
        }

        await onOnboardingComplete(privateKey, password, { 0: true }, 'import-social', userInfo?.email);
      } else {
  ...
  return (
    <ImportWalletContext.Provider
      value={{
        ...
        //getLedgerAccountDetails,
        getLedgerAccountDetailsForIdxs,
        walletAccounts: isLedger ? ledgerWalletAccounts.concat(ledgerCustomWalletAccounts) : hotWalletAccounts,
        currentStepName,
        socialLogin,
        customWalletAccounts,
        getCustomLedgerAccountDetails,
