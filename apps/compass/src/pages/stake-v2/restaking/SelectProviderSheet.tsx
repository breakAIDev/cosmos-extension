...
export const ProviderCard = observer(({ provider, onClick, rootDenomsStore }: ProviderCardProps) => {
        ...
        <div className='flex flex-col justify-center items-start w-full'>
          <div className='flex justify-between w-full'>
            <Text size='sm' color='text-black-100 dark:text-white-100' className='font-bold mb-0.5'>
              {sliceWord(
                provider.moniker,
                isSidePanel() ? 22 + Math.floor(((Math.min(window.innerWidth, 400) - 320) / 81) * 7) : 30,
                0,
              )}
            </Text>
...
export default function SelectProviderSheet({
  ...
  const renderItem = useCallback(
    (_: number, item: Provider | { itemType: 'inactiveHeader' }) => {
      ...
      return <ProviderCard provider={item} onClick={() => onProviderSelect(item)} rootDenomsStore={rootDenomsStore} />;
    },
    [onProviderSelect],
  );
    ...
    >
      <div className='flex flex-col gap-y-6 h-full'>
        ...
        <div className='w-full h-full' style={{ overflowY: 'scroll' }}>
          ...
          {!isLoading && listItems.length > 0 && (
            <Virtuoso data={listItems} itemContent={renderItem} style={{ flexGrow: '1', width: '100%' }} />
          )}
        </div>
      </div>
      <SelectSortBySheet
