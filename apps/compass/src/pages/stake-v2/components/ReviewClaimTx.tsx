...
const ReviewClaimTx = observer(({ isOpen, onClose, validator, validators, setClaimTxMode }: ReviewClaimTxProps) => {
  ...
  const validatorDetails = useMemo(() => {
    ...
    const imgSrc = imageUrl ?? (rewardValidators && rewardValidators[0]?.image) ?? Images.Misc.Validator;
