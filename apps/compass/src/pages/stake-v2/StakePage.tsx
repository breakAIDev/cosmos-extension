...
const StakePage = observer(() => {
  ...
  const isLoadingAll = loadingDelegations || loadingNetwork || loadingRewards || loadingUnboundingDelegations;
  ...
  if (!activeWallet) {
    return (
      <div className='relative w-full overflow-clip panel-height flex items-center justify-center'>
        <EmptyCard src={Images.Logos.CompassCircle} heading='No wallet found' logoClassName='size-14' />
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <SelectWallet isVisible={showSelectWallet} onClose={() => setShowSelectWallet(false)} title='Wallets' />

      <StakeTxnSheet mode={claimTxMode} isOpen={!!claimTxMode} onClose={() => setClaimTxMode(null)} />
