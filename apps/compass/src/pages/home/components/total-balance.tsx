...
export const TotalBalance = observer(() => {
  ...
  const isSeiEvmChain = useIsSeiEvmChain(activeChain === AGGREGATED_CHAIN_KEY ? 'seiTestnet2' : activeChain);

  const countRef = useRef(0);
  const [timedBalancesFiatValue, setTimedBalancesFiatValue] = useState(rootBalanceStore.totalFiatValue);

  const totalFiatValue = useMemo(() => {
    const addEvmDetails = hasToAddEvmDetails(isSeiEvmChain, addressLinkState, isEvmOnlyChain ?? false);
