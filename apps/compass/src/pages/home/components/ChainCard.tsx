    ...
    >
      <div className='flex items-center flex-1 gap-2'>
        {showStars && (
          <>
            {isStarred ? (
              <Star size={16} weight='fill' className='text-yellow-500 cursor-pointer' onClick={onStarToggle} />
            ) : (
              <Star size={16} className='text-gray-200 dark:text-gray-800 cursor-pointer' onClick={onStarToggle} />
            )}
          </>
        )}
        ...
        <Text
          ...
          className='font-bold'
          color={
            isWatchWalletNotAvailableChain ? 'dark:text-gray-800 text-gray-200' : 'dark:text-white-100 text-black-100'
          }
      ...
      <div className='ml-auto flex items-center'>
        {selectedChain === chainName ? (
          <CheckCircle
            ...
            className='ml-2'
            style={{
              color: selectedChain === AGGREGATED_CHAIN_KEY ? Colors.green600 : Colors.getChainColor(selectedChain),
            }}
          />
        ) : null}
        {beta && (
