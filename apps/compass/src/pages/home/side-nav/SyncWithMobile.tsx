...
function EnterPasswordView({ setRevealed, setPassword, setQrData, goBack }: EnterPasswordViewProps): ReactElement {
  ...
  return (
    <div className='panel-height enclosing-panel'>
      <Header title='Sync with mobile app' action={{ type: HeaderActionType.BACK, onClick: goBack }} />
...
function QrCodeView({ qrData, setRevealed, setPassword, goBack }: EnterPasswordViewProps): ReactElement {
  return (
    <div className='panel-height enclosing-panel'>
      <Header title='Sync with mobile app' action={{ type: HeaderActionType.BACK, onClick: goBack }} />
      <div className='relative flex items-center justify-center flex-col'>
        ...
        <div className='text-sm dark:text-gray-400 text-gray-800 my-4 mx-10 text-center'>
          Scan the <span className='font-bold dark:text-gray-200 text-gray-800'>QR code</span> using the{' '}
          <span className='font-bold dark:text-gray-200 text-gray-800'>Leap Cosmos mobile app</span> to sync your
          primary accounts and settings
        </div>
      </div>
    </div>
  );
...
export default function SyncWithMobile({ goBack }: { goBack: () => void }): ReactElement {
  ...
  return isRevealed && password !== '' && qrData.length > 0 ? (
    <QrCodeView qrData={qrData} setRevealed={setRevealed} setPassword={setPassword} goBack={goBack} />
  ) : (
    <EnterPasswordView setRevealed={setRevealed} setPassword={setPassword} setQrData={setQrData} goBack={goBack} />
  );
}
