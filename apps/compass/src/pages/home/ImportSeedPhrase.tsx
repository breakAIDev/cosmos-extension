  ...
  return (
    <BottomModal
      ...
      footerComponent={
        <>
          ...
          <Button size='md' disabled={!secret || !!error || isLoading} onClick={handleImportWallet} className='flex-1'>
            Import Wallet
          </Button>
        </>
      }
    >
      <div className='flex flex-col gap-y-4 items-center justify-center'>
        <SeedPhraseInput onChangeHandler={onChangeHandler} isError={!!error} onPage='SelectWallet' />

        {error && <span className='text-xs font-medium text-destructive-100 block text-center'>{error}</span>}
      </div>
    </BottomModal>
  );
});
