...
export const useCustomAddedERC20TokensStore = create<CustomAddedERC20TokensStore>((set) => ({
  ...
  setCustomAddedERC20Tokens: (customAddedERC20Tokens: LifiSupportedAsset[]) => set(() => ({ customAddedERC20Tokens })),
}));

export function useSetCustomAddedERC20Tokens() {
  const setCustomAddedERC20Tokens = useCustomAddedERC20TokensStore((state) => state.setCustomAddedERC20Tokens);
