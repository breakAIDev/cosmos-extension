...
export function useEnableToken(
  ...
  const betaERC20Denoms = betaERC20DenomsStore.getBetaERC20DenomsForChain((chain?.key ?? '') as SupportedChain);
  ...
  const managedTokens = useMemo(() => {
    ...
    if (cw20Tokens) {
      _managedTokens = _managedTokens.concat(Object.values(cw20Tokens)?.map((token) => token.coinMinimalDenom));
    }
    ...
    if (betaCw20Tokens) {
      _managedTokens = _managedTokens.concat(Object.values(betaCw20Tokens)?.map((token) => token.coinMinimalDenom));
    }
  ...
  const enableToken = useCallback(async () => {
    ...
    if (tokenToEnable?.skipAsset?.evmTokenContract || tokenToEnable?.coinMinimalDenom?.startsWith('0x')) {
      const evmAddress = tokenToEnable?.skipAsset?.evmTokenContract ?? tokenToEnable?.coinMinimalDenom;
      ...
      const isAbsentInBetaERC20Denoms =
        !betaERC20Denoms ||
        !(Object.values(betaERC20Denoms)?.map((token) => token.coinMinimalDenom) ?? []).includes(evmAddress);
      const isAbsentInERC20Denoms =
        !erc20Denoms ||
        !(Object.values(erc20Denoms)?.map((token) => token.coinMinimalDenom) ?? []).includes(evmAddress);
      ...
      if (!seiAddress || seiAddress?.startsWith('0x')) {
        ...
        isAbsentInRootDenoms =
          !rootDenoms ||
          !(Object.values(rootDenoms)?.map((token) => token.coinMinimalDenom) ?? []).includes(seiAddress);
        isAbsentInCW20Denoms = true;
        isAbsentInBetaCW20Denoms = true;
        isAbsentInEnabledCW20Denoms = true;
      } else {
        ...
        isAbsentInCW20Denoms =
          !cw20Tokens ||
          !(Object.values(cw20Tokens)?.map((token) => token.coinMinimalDenom) ?? []).includes(seiAddress);

        isAbsentInBetaCW20Denoms =
          !betaCw20Tokens ||
          !(Object.values(betaCw20Tokens)?.map((token) => token.coinMinimalDenom) ?? []).includes(seiAddress);

        isAbsentInEnabledCW20Denoms = !enabledCW20Tokens || !(enabledCW20Tokens ?? []).includes(seiAddress);
      }
      ...
      if (
        ...
        const tokenInfo: NativeDenom = {
          ...
          coinDecimals:
            tokenToEnable?.skipAsset?.evmDecimals ?? tokenToEnable.coinDecimals ?? tokenToEnable.skipAsset?.decimals,
          chain: chain?.key,
        ...
        await rootBalanceStore.erc20BalanceStore.fetchERC20TokenBalances(chain.key, SWAP_NETWORK, [evmAddress], false, {
    ...
    const _disabledCW20Tokens = disabledCW20Tokens.filter((_token) => _token !== tokenToEnable?.coinMinimalDenom);
