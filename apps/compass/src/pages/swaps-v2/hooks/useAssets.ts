...
export default function useAssets() {
  ...
  const { data: _allSkipAssets, isLoading: loadingAllSkipAssets } = useAllSkipAssets(useAllSkipAssetsParams);

  const { data: seiLifiAssets } = useLifiAssets(
    isLifiEnabled ? String(compassSeiEvmConfigStore.compassSeiEvmConfig.PACIFIC_ETH_CHAIN_ID) : undefined,
    {
  ...
  const allAssetsKey = useMemo(() => {
    ...
    return ['allAssets', seiLifiAssets, allSkipAssets, customAddedERC20Tokens, allDenoms, blacklistedTokens];
  ...
  const { data: allAssets, isLoading: loadingAllAssets } = useQuery(
    ...
    async () => {
          ...
          Object.keys(allSkipAssets).forEach((chainId) => {
            if (allSkipAssets[chainId]) {
              allSkipAssets[chainId] = allSkipAssets[chainId]?.filter(
                (asset) =>
                  !blacklistedTokens.includes(asset.originDenom?.replace(/(cw20:|erc20\/)/g, '').toLowerCase()),
              );
            }
          });
      ...
      const combinedCompassTokenDenomInfo = Object.assign({}, compassTokenTagsStore.compassTokenDenomInfo, allDenoms);
      ...
      await Promise.allSettled(
        seiSkipAssets.map(async (skipAsset) => {
          ...
          skipAsset.coingeckoId = combinedCompassTokenDenomInfo[evmContract]?.coinGeckoId ?? skipAsset.coingeckoId;
        ...
        ?.map((lifiAsset) => {
          ...
          if (evmTokenContract === AddressZero) {
            ...
            return {
              ...
              coingeckoId: combinedCompassTokenDenomInfo[originDenom]?.icon ?? lifiAsset.coingeckoId,
            };
          }
          return {
            ...lifiAsset,
            ...
            coingeckoId: combinedCompassTokenDenomInfo[evmTokenContract]?.icon ?? lifiAsset.coingeckoId,
          };
        });
        ...
        Object.keys(_allAssets).forEach((chainId) => {
          if (_allAssets[chainId]) {
            _allAssets[chainId] = _allAssets[chainId]?.filter(
              (asset) => !blacklistedTokens.includes(asset.originDenom?.replace(/(cw20:|erc20\/)/g, '').toLowerCase()),
            );
          }
        });
