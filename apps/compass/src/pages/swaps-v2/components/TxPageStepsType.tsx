...
export function TxPageStepsType({
  ...
  const { data: destDenomData, isLoading: isDestDenomLoading } = useDenomData(destAsset, destChainId);
  ...
  const handleViewInExplorer = useCallback(
    (chainId: string, txHash: string) => {
      if (isCompassWallet()) {
        const explorerLink = response?.packetTxs?.sendTx?.explorerLink ?? response?.packetTxs?.receiveTx?.explorerLink;
        if (explorerLink) {
  ...
  const { status, txData } = useMemo(() => {
    ...
    if (!packetTxs) {
      _status = actionIndex === 0 || transferSequenceIndex !== prevTransferSequenceIndex ? response?.state : undefined;
