...
export function ConversionRateDisplay({ onClick, isReviewSheet }: ConversionRateDisplayProps) {
  const { amountOut, inAmount, sourceToken, destinationToken, loadingRoutes, loadingMessages, isSkipGasFeeLoading } =
    useSwapContext();
  const [interchangeTokens, setInterchangeTokens] = useState<boolean>(false);
  const isFirstLoadRef = useRef(true);

  const loading = isFirstLoadRef.current ? loadingRoutes || loadingMessages || isSkipGasFeeLoading : loadingRoutes;
  ...
  return loading ? (
    ...
    <button onClick={onClick ?? handleInterchange} className='flex items-center gap-1 flex-wrap'>
      {conversionRate && baseToken && quoteToken && (
        <>
          <span className={cn('text-secondary-800 text-xs !leading-[18.9px] font-medium', isReviewSheet && 'text-sm')}>
            1 {baseToken?.symbol} ={' '}
          </span>
          <div className={cn('!leading-[18.9px] text-xs font-medium h-[19.2px]', isReviewSheet && 'text-sm')}>
            <span className='text-secondary-800'>
              {formatTokenAmount(conversionRate ?? '0', sliceWord(quoteToken?.symbol ?? '', 4, 4), 3)}
            </span>
            {baseToken.usdPrice && (
              <span className='ml-1 text-muted-foreground'>({formatCurrency(new BigNumber(baseToken.usdPrice))})</span>
            )}
          </div>
        </>
      )}
