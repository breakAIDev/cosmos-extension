...
export const ActivityList = (props: { txResponse: TxResponse; setSelectedTx: (tx: SelectedTx) => void }) => {
  ...
  const accountExplorerLink = useMemo(() => {
    if (chains[selectedChain]?.txExplorer?.[activeNetwork]?.accountUrl) {
      ...
      return `${removeTrailingSlash(chains[selectedChain]?.txExplorer?.[activeNetwork]?.accountUrl ?? '')}/${address}`;
    }
  ...
  return (
    <div className='flex flex-col flex-1 w-full gap-6 px-6 pb-6 pt-8 mb-20 overflow-auto'>
      ...
      {!txResponse?.loading && accountExplorerLink ? <CheckMoreLink href={accountExplorerLink} /> : null}
    </div>
  );
};
