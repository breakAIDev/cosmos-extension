...
import { convertLifiErrorToDisplayError, convertLifiStatusToTxnStatus } from 'pages/swaps-v2/utils/lifiTracking';
    ...
    return {
      ...
      response: data
        ? {
            ...
            error: convertLifiErrorToDisplayError(data.status, data.substatus, data.substatusMessage),
      ...
      while (true) {
        ...
        if (txnStatus.success) {
          ...
          const isFailedLifiTxn =
            pendingTx.routingInfo?.aggregator === RouteAggregator.LIFI && state === TXN_STATUS.FAILED;
