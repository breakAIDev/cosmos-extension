...
import { chainIdToChain, ChainInfo, isAptosChain, sleep, SupportedChain } from '@leapwallet/cosmos-wallet-sdk';
...
async function getSidePanelStatus() {
  ...
  try {
    ...
    isSidePanelMounted = response?.type === 'side-panel-status' && response?.message?.enabled === true;
  } catch (e) {
...
export async function awaitApproveChainResponse(payloadId: string) {
  return new Promise((resolve, reject) => {
    const listener = async (message: { type: string; payload: any; status: string }, sender: any) => {
      ...
      if (message.type === 'chain-enabled' && message.payload.payloadId === payloadId) {
        ...
        //browser.runtime.onMessage.removeListener(listener)
      } else if (message.type === 'chain-approval-rejected' && message.payload.payloadId === payloadId) {
