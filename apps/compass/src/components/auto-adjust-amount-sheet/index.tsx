...
> = ({ isOpen, tokenAmount, feeAmount, setAmount, nativeDenom, onAdjust, onCancel, decimalsToUse }) => {
  ...
  return (
    <BottomModal isOpen={isOpen} onClose={onCancel} title='Adjust for transaction fees' className='!px-6 !py-7'>
      <p className='text-secondary-800 text-sm font-medium mb-8'>
        ...
        transaction fees. Should we auto-adjust the amount?
      </p>

      <div className='flex items-center rounded-lg bg-secondary-100 mb-10 p-5'>
        <div className='flex-1 text-center text-sm text-monochrome font-bold'>{displayTokenAmount}</div>

        <ArrowRight size={32} className='text-monochrome bg-secondary-300 p-2 rounded-full' />
        <div className='flex-1 text-center text-sm text-green-500 font-bold'>{displayUpdatedAmount}</div>
      </div>
...
> = ({ isOpen, tokenAmount, tokenBalance, feeAmount, setAmount, nativeDenom, onAdjust, onCancel, decimalsToUse }) => {
  ...
  return (
    <BottomModal isOpen={isOpen} onClose={onCancel} title='Adjust for transaction fees' className='!px-6 !py-7'>
      <p className='text-secondary-800 text-sm font-medium mb-8'>
        You seem to have insufficient {nativeDenom?.coinDenom ?? ''} balance to pay transaction fees. Should we
        auto-adjust the amount?
      </p>
