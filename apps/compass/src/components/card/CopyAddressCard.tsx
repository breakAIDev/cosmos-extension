...
const CopyAddressCard = React.memo(({ forceChain, address, forceName }: CopyAddressCardProps) => {
  ...
  const name = useMemo(() => {
    ...
    let _name =
      activeChainInfo.addressPrefix.slice(0, 1).toUpperCase() + activeChainInfo.addressPrefix.slice(1).toLowerCase();
      ...
      <AnimatePresence mode='wait'>
        ...
        >
          <motion.span
            ...
            exit='hidden'
          >
            {isCopied ? <Check className='size-4 text-accent-blue-200' /> : <CopyIcon className='size-4' />}
          </motion.span>
        </Button>
      </AnimatePresence>
    </div>
