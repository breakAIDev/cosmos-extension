...
const SelectWalletsWalletCard = ({ imgSrc, onClick, title, subtitle, isSelected }: SelectWalletsWalletCardProps) => {
...
export const SelectWallets = ({ selectedWallets, setSelectedWallets, forceChain }: SelectWalletsProps) => {
      ...
      <div>
        {walletsList
          ...
          .map((wallet, index, array) => {
            ...
            const walletLabel = wallet.walletType === WALLETTYPE.LEDGER ? ` · /0'/0/${wallet.addressIndex}` : '';
            ...
            return (
              <>
                <SelectWalletsWalletCard
                  key={wallet.name}
                  subtitle={wallet.addresses[chain] ? `${sliceAddress(wallet.addresses[chain])}${walletLabel}` : ''}
