...
const getBalance = async ({ address, lcdUrl, chain }: { address: string; lcdUrl: string; chain: string }) => {
  try {
...
const WalletInfoCard = ({
  ...
  return (
    <label
      {...props}
      htmlFor={`${path}-${id}`}
      className={cn('flex flex-row gap-3 items-center p-5 w-full cursor-pointer select-none', className)}
    >
