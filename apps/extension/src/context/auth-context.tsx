      ...
      if (!password) {
        ...
        try {
          ...
          if (!storage[V118_KEYSTORE_MIGRATION_COMPLETE]) {
            const newStore = await browser.storage.local.get([ACTIVE_WALLET, KEYSTORE, ACTIVE_CHAIN]);
          ...
          if (!storage[V125_BETA_NFT_COLLECTIONS_MIGRATION_COMPLETE]) {
            const storedBetaNftCollections = await browser.storage.local.get([BETA_NFTS_COLLECTIONS]);
...
const RequireAuthView = ({
  ...
  return views.length === 0 ? (
    <ExtensionPage titleComponent={titleComponent}>
      <div
        className={classNames('absolute top-0 rounded-2xl flex bottom-0 z-5 justify-center items-center', {
          'panel-height max-w-full': isSidePanel(),
          'w-1/2': !isSidePanel(),
        })}
      >
        <div
          ...
          className={classNames('relative panel-height', {
            'dark:shadow-sm shadow-xl dark:shadow-gray-700': !location.pathname.includes('onboardEvmLedger'),
