...
declare const self: Window & typeof globalThis & { lumina: any; luminaClient: any; luminaInPopup: any };
...
export class LightNode {
  ...
  async updateRunningTimeForLightNode() {
    const intervalId = setInterval(() => {
      browser.storage.local.get([LIGHT_NODE_RUNNING_STATS, IS_LIGHT_NODE_RUNNING]).then((storage) => {
        ...
        if (isLightNodeRunning && lightNodeRunningStats) {
          ...
          browser.storage.local.set({
            [LIGHT_NODE_RUNNING_STATS]: JSON.stringify({
              ...
              totalRunningTimeInMilliSeconds: totalRunningTimeInMilliSeconds + LIGHT_NODE_TIME_CAPTURE_INTERVAL,
            }),
          });
        }
