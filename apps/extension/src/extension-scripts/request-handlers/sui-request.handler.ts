...
type OpenPopupFunction = (page: Page, queryString?: string, response?: [string, any, number]) => Promise<void>;
  ...
  const getSuiChains = () => {
    try {
      ...
      Object.values(allChains).forEach((chain) => {
        ...
        if (!!chain.testnetChainId && chain.testnetChainId !== chain.chainId && isSuiChain(chain.testnetChainId)) {
  ...
  switch (method) {
      ...
      try {
        ...
        if (!silent) {
          try {
            ...
            if (response === 'popup-closed' || (typeof response === 'object' && response.status !== 'success')) {
              sendResponse(`on${method?.toUpperCase() ?? ''}`, { error: 'Connection rejected by user' }, payload.id);
              return;
            }
          } catch (e) {
            sendResponse(`on${method?.toUpperCase() ?? ''}`, { error: 'Connection rejected by user' }, payload.id);
    ...
    case SUI_FEATURE.SUI__REPORT_TRANSACTION_EFFECTS: {
      try {
        ...
        sendResponse(`on${method?.toUpperCase() ?? ''}`, { error: 'Failed to report transaction effects' }, payload.id);
      }
      break;
    }
  }
