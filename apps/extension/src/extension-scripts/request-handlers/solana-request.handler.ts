  ...
  const getSolanaChains = () => {
    try {
      ...
      Object.values(allChains).forEach((chain) => {
        ...
        if (!!chain.testnetChainId && chain.testnetChainId !== chain.chainId && isSolanaChain(chain.testnetChainId)) {
  ...
  switch (method) {
    ...
    case SOLANA_METHOD_TYPE.CHANGE_NETWORK: {
      try {
        ...
        sendResponse(`on${method.toUpperCase()}`, { success: { chainId: payload.network } }, payload.id);
      } catch (error: any) {
        sendResponse(
          `on${method.toUpperCase()}`,
          {
