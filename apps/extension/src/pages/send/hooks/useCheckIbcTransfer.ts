  ...
  useEffect(() => {
    async function checkAddressAndIBCTransfer() {
      ...
      if (isBitcoinChain(sendActiveChain)) {
        if (!isValidBtcAddress(selectedAddress.address, sendSelectedNetwork === 'mainnet' ? 'mainnet' : 'testnet')) {
      ...
      if (activeChainInfo?.evmOnlyChain) {
        if (!isEthAddress(selectedAddress.address) || !isEthAddress(selectedAddress?.ethAddress ?? '')) {
          setAddressError('The entered address is invalid');
        } else {
          setAddressError(undefined);
        }
      ...
      const defaultChannelId = await getDefaultChannelId(sendActiveChain, destinationChain as SupportedChain, chains);
