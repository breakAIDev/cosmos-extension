...
function BitcoinTransaction({ txnData }: BitcoinTransactionProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('beforeunload', () => handleRejectClick(navigate, txnData?.payloadId));
    };
  ...
  switch (txnData.signTxnData.methodType) {
    case BITCOIN_METHOD_TYPE.SEND_BITCOIN:
      return <SendBitcoin txnData={txnData} rootDenomsStore={rootDenomsStore} rootBalanceStore={rootBalanceStore} />;
