...
export const SideNavSection = observer(({ children, className }: { children: ReactNode; className?: string }) => {
  return (
    <section
      className={cn(
...
const SideNav = observer((): ReactElement => {
  ...
  return (
    <Drawer direction='left' open={isShown} onOpenChange={(open) => !open && globalSheetsStore.toggleSideNav()}>
      <DrawerContent
        showHandle={false}
        className={'panel-width panel-height overflow-clip overflow-y-auto rounded-none bg-secondary-50 isolate'}
      >
        ...
        <GeneralSecurity isVisible={showNavPage === NavPages.Security} goBack={() => setShowNavPage(undefined)} />

        <NetworkDropUp isVisible={showNavPage === NavPages.Network} goBack={() => setShowNavPage(undefined)} />

        <Preferences isVisible={showNavPage === NavPages.Preferences} goBack={() => setShowNavPage(undefined)} />
        ...
        <SyncWithMobile open={showNavPage === NavPages.SyncWithMobile} goBack={() => setShowNavPage(undefined)} />
      </DrawerContent>
    </Drawer>
  );
});
