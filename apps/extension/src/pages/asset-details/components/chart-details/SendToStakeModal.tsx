...
const SendToStakeModal = observer(({ isVisible, onClose, ibcDenom, nativeDenom }: SendToStakeModalProps) => {
  ...
  return (
    <BottomModal isOpen={isVisible} onClose={onClose} title={`Stake on ${nativeChainName.chainName}`} className='p-6'>
      <div className='flex flex-col gap-y-4 px-1'>
        <div className='flex rounded-xl dark:bg-gray-950 bg-gray-100 border dark:border-gray-850 border-gray-200 items-center justify-evenly px-5 py-6'>
          <div className='relative w-16 h-16 flex items-center justify-center'>
            <img src={ibcDenom.img} onError={imgOnError(defaultIconLogo)} className='w-12 h-12 rounded-full' />
            <img
              src={
        ...
        <div className='font-medium text-md text-gray-800 dark:text-gray-200'>
          ...
          <span className='font-bold inline text-black-100 dark:text-white-100'>{ibcDenom.symbol}</span>
          &nbsp;to&nbsp;
          <span className='font-bold inline text-black-100 dark:text-white-100'>{nativeChainName.chainName}</span> to
          start staking.
        </div>
        <Buttons.Generic className='w-full' size='normal' color={Colors.green600} onClick={handleSendToStake}>
