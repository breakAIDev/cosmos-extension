...
export function ShowVotes({ dataMock, chain }: IShowVotes) {
  ...
  return (
    <div className='h-[52px]'>
      <div className='flex items-start no-scrollbar overflow-y-auto whitespace-nowrap' ref={scrollRef} {...props}>
        {dataMock.map((item) => {
          ...
          return (
            <div
              key={item.color}
              className={`px-3 py-2 dark:bg-gray-900 bg-white-100 rounded-[12px] mr-3${isLoading ? ' w-[150px]' : ''}`}
            >
              {isLoading ? (
                <Skeleton count={2} />
              ) : (
                <>
                  <div className='flex items-center'>
                    <div className='rounded-[2px] w-3 h-3 mr-1' style={{ backgroundColor: item.color }} />
                    <p className='dark:text-white-100 text-gray-400 whitespace-nowrap text-xs font-bold'>{`${item.title} - ${item.percent}`}</p>
                  </div>
                  <Text size='xs' color='text-gray-400 font-medium whitespace-nowrap' style={{ lineHeight: '20px' }}>
