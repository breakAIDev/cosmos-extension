...
export function useExecuteTx({
  ...
  }, [gasPriceOptions, gasOption, gasEstimate, userPreferredGasLimit, userPreferredGasPrice, sourceChain, feeDenom]);
  ...
  const getSwapFeeInfo = useCallback(async () => {
    ...
    const amountValue = feeAmountValue
      ? (await getTxnLogAmountValue(feeAmountValue.toString(), {
          ...
          chain: Object.values(chainInfos).find((chain) => chain.chainId === swapFeeDenomInfo?.originChainId)
  ...
  }, [routingInfo.aggregator, routingInfo?.messages, routingInfo.route, executeSkipTx, executeMosaicTx]);
