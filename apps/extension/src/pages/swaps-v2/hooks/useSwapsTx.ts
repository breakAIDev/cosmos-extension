...
export function useSwapsTx({
  ...
  const sourceTokensLoading = sourceChain ? getLoadingStatusForChain(sourceChain?.key, SWAP_NETWORK) : false;
  ...
  useEffect(() => {
    ...
    const isZeroState = !highestBalanceToken?.amount || new BigNumber(highestBalanceToken.amount).isEqualTo(0);
      ...
      destinationTokenToSet = destinationAssets.find(
        (asset) =>
          (asset.ibcDenom === searchedDestinationToken ||
            ...
            (hasCoinType(asset.skipAsset) &&
              asset.skipAsset.coinType?.toLowerCase() === searchedDestinationToken?.toLowerCase()) ||
            getKeyToUseForDenoms(asset.skipAsset.denom, asset.skipAsset.originChainId) ===
              searchedDestinationToken?.replace(/(cw20:|erc20\/)/g, '') ||
    ...
    const sourceTokenChain = chainsToShow.find((chain) => chain.chainId === sourceTokenToSet?.skipAsset?.chainId);
  ...
  useEffect(() => {
    ...
    if (
      ...
      (destinationToken?.chain !== 'movement' ||
        (sourceToken?.skipAsset?.denom && destinationToken?.skipAsset?.denom === sourceToken?.skipAsset?.denom))
    ) {
      ...
      const destToken = destinationAssetsData?.find(
        (asset) => asset.chain === 'movement' && asset.coinMinimalDenom !== sourceToken.coinMinimalDenom,
      );
      if (!destToken) return;
