...
export function useAggregatorGasFeeSWR(
  ...
  const [aptosGasDetails, setAptosGasDetails] = useState<{ gasEstimate: string; gasUnitPrice: string } | undefined>();
  ...
  const { evmJsonRpc, lcdUrl } = useChainApis((sourceChain?.key ?? '') as SupportedChain, SWAP_NETWORK);
  ...
  useSWR(
    enabled && !!routeResponse?.response && routeResponse.aggregator === RouteAggregator.MOSAIC
      ...
      : null,
    async function calculateGasFee() {
      if (!routeResponse || (routeResponse && routeResponse?.aggregator !== RouteAggregator.MOSAIC)) {
        throw new Error('missing data');
      }
  ...
  return {
    ...
    isLoading:
      routeResponse?.aggregator === RouteAggregator.MOSAIC && !aptosGasDetails ? true : skipGasFeeSWRResponse.isLoading,
  };
}
