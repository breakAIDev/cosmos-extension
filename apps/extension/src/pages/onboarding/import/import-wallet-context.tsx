...
export type ImportWalletType = 'seed-phrase' | 'private-key' | 'ledger' | 'watch-wallet' | 'evm-ledger';
...
export const ImportWalletProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const isPrivateKey = walletName?.toLowerCase().includes('private') || isMetamask || isOtherEvmWallets;
  ...
  useEffect(() => {
    if (walletAccounts?.length && Object.keys(selectedIds).length === 0) {
      ...
      setSelectedIds({
        [firstWallet.index]: !!firstWallet.address && !existingAddresses?.includes(firstWallet.address),
      });
    }
